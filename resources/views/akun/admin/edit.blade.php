        ...
        <div class="pd-ltr-20 xs-pd-20-10">
            <div class="min-height-200px">
                ...
                <div class="pd-20 card-box mb-30">
                    <div class="clearfix">
                        <div class="pull-left">
                            <h4 class="text-dark h4">Edit Akun admin</h4>
                            <p class="mb-30">Isi data dengan benar</p>
                        </div>
                    </div>
                    <form action="{{ route('sudo.akun_admin.update', $admin->id) }}" method="POST">
                        @method('PUT')
                        @csrf
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="custom-select2 form-control" style="width: 100%; height: 38px"
                                        ...
                                        <option value="Aktif" {{ $admin->status == 'Aktif' ? 'selected' : '' }}>
                                            Aktif</option>
                                        <option value="Alumni" {{ $admin->status == 'Alumni' ? 'selected' : '' }}>
                                            Pensiun</option>
                                        <option value="Pensiun" {{ $admin->status == 'Drop Out' ? 'selected' : '' }}>
                                    ...
                                    <select class="custom-select2 form-control" multiple="multiple" style="width: 100%"
                                        ...
                                        <optgroup label="Role">
                                            <option value="admin lab"
                                                {{ $account->hasRole('admin lab') ? 'selected' : '' }}>Admin Lab</option>
                                            <option value="admin berkas"
                                                {{ $account->hasRole('admin lab') ? '' : 'selected' }}>Admin Berkas
                                            </option>
